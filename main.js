...
var visualizeTextureWGSL = `@group(0) @binding(0) var mySampler : sampler;
...
const outBlockBits = document.getElementById('outBlockBits');
...
const kDefaultFormat = 'etc2-rgb8unorm';
const kLargestBlockByteSize = Math.max(...Object.values(kCompressedFormatInfo).map((info) => info.blockByteSize));
const parameters = {
    'etc2-rgb8unorm': {
        mode: 'individual',
        R1: 0,
...
const dataFromBits = (() => {
    ...
    return function dataFromBitsString(bits) {
        let numBits = 0;
        let byte = 0;
